...
| 套 | 样本 | 曾经在哪 |
| --- | --- | --- |
| A 产物命名 | `A1-fastv3.3.toml` | 入库产物目录、`BUILTIN_PRESETS` |
| B 云端历史命名 | `A1F_260628.toml` | 对照基线目录、机型定义的 `presetFile`、IR 夹具 |
| C 消费端期望 | `A1-fastv3.3.toml` | 发布路径（多做一次 `to_lowercase()`） |

A 与 C 形状相同但曾是两份独立实现，B 与 C 对不上——**后果不是报错，是消费端找不到文件**。
本规范取 A/C 的形状作为唯一标准，B 只作为迁移输入，映射表见 spec 的 Task 4。
