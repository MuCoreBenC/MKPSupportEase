...
pub mod disabled {
    ...
    /// 机型没配床身尺寸 —— **生成出来也用不了**，所以不走「暂无资源」那句。
    ///
    /// 这两句话必须分开：「暂无资源」让人去找资源，而这一条要补的是尺寸。
    /// 指错方向的提示比没有提示更费时间（b04 P0 审计查明：消费端在内置尺寸表里
    /// 查不到这台机型时会拒掉整份配方）
    pub const BUILD_NO_DIMENSIONS: &str =
        "这台机型还没配床身尺寸，生成出来消费端会整份拒掉 —— 先把尺寸补上";
    /// 草稿干净时保存 / 丢弃都不能点
