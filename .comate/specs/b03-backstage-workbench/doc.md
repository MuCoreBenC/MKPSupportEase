...
照参考那份 tokens 建一份我们自己的（值可以照抄，它是同一个产品群的设计系统）：控件高度 26/30/34、徽章圆角 2px、卡片 12px、浮层 16px、文字四级、来源三档色（灰=出厂 / 紫=机型 / 蓝=版本）、`--w-warn` 橙 = **"值变了 / 要留意"不是错误**、`--w-danger` 红 = 真的错了。

> **修正（b03-matrix-usability）：草稿标记从「蓝色描边」改成「左侧琥珀竖条」，蓝色专用于「选中」。**
>
> 原本两者同色（选中用淡蓝底+蓝边，草稿用蓝色 `inset` 描边），实际点起来的后果是
> 「取消选中之后还剩一圈蓝」看着像取消没生效。而琥珀本来就是这套 tokens 里
> 「值变了 / 要留意」的颜色，草稿未保存正是这个意思。
>
> 竖条而不是整格描边还顺带解决一件事：一列改了 28 项时 28 个框会连成一片，
> 而 28 条竖条是一道能一眼扫完的垂直轨迹。

另外两条同一批定下的呈现纪律：

- **一次点击不许让任何东西移动。** 选中提示常驻占位不按需插入；列头的状态字**原位替换**不新增行；单元格里文本与控件共用 `--w-cell-h`。上一稿这三处都会让整表下移一截，于是下一次点击必然点偏。
- **被禁用的格子照样要能点。** `disabled` 的按钮连 tooltip 都弹不出来，等于让人猜。被上级条件关着时点它，展开的是「由『X』控制，需 Y」加一个跳过去的按钮。

密度用 `ResizeObserver` 量**容器实测宽度**（不是 media query），四档 `ultra/wide/compact/mini`，断点 1600/1080/640。mini 档纪律：**不许藏掉任何可用操作**，只能改布局方向与隐藏说明文字。`--w-cell-h` **不随密度变** —— 行高一随密度变，位移那条纪律就从密度切换那边破了。
...
**冻结的那三种必须自己不透明**：背景一旦换成半透明色（比如选中的淡蓝 10% alpha），下面滚过去的行就会从里面透出来，界面上表现为「文字叠在表头上」。所以选中态用 `inset` 阴影叠色，不替换背景。

**行序是四段键，由 Rust 排**（b03-matrix-usability）：

```
tab.order → section.order → 父项的 layout.order → 自己的 layout.order → key
```

只按 `layout.order` 排是错的 —— 那是 section **内部**的序号（各组都从 1 开始，还有 `0.5` 这种插队值），当全局键用等于把 16 个分组洗牌。第三段取**父项的**序号而不是单独排一段 `depth`，否则一个有子项的父行会被自己的子项挤到组尾。最后一段 key 兜底，保证同序时也稳定。

前端照 `Row.sectionLabel` 的变化插分组表头行，**不自己聚合**；搜索状态下不插（命中散落在各组，会变成一堆只有一行的组），改在行头显示组名。
...
| 3 | `uiComponent == switch` | **常驻真开关**，点一下直接落一条撤销（不用先选中） |
| 4 | 其他 | 只读文本 + 来源徽章；被选中时升级成真控件 |

**"不适用"与"被关着"是两件不同的事**，必须分开说：前者是这台机型根本没有这一项，后者是有但被上级字段的条件关掉了 —— **看得见、改不动，并且写明由谁控制**。

被关着时的三件事（b03-matrix-usability，反馈原话「我都不知道是哪个选项导致它灰色」）：

1. **行头常驻一句「受『X』控制」**，与当前值无关 —— 格子还没变灰它就在那儿了
2. 格子**照样能点**，点开的是「由『X』控制，需 Y」加一个「去改那一项」
3. 「去改那一项」会先把分类/搜索的过滤让开，再选中并滚到那一格；控制它的字段在这台机型上不存在时**不给按钮**，改成一句说明

有 `parentKey` 的行缩进一级并写「属于：X」—— 父子平铺成两行毫不相干的行，是「没按逻辑排序」的另一半原因。
